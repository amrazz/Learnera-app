...
const Sidebar = ({ activeMenu, setActiveMenu, isMobile }) => {
  ...
  return (
    <div
      className={`text-white ${isToggled ? "w-32" : "w-64"} ${
        activeMenu ? "h-screen" : "h-full"
      } flex flex-col p-5 shadow-lg transition-all duration-300`}
      style={{
        background:
    ...
    >
      ...
      <div className={`${!isToggled ? "flex items-center justify-between mb-8" : "flex justify-center pb-5"}`}>
        {!isToggled ? (
          <div className="flex items-center justify-between w-full">
            <span className="text-3xl font-extrabold tracking-wide">LEARNERA</span>
      ...
      {/* Menu Section with an inner wrapper to adjust the scrollbar */}
      <div
        className="flex-1 relative overflow-x-hidden overflow-y-hidde"
        style={{
          height: "calc(100vh - 120px)", // Account for header and padding
        }}
      >
        {/* This inner div is our scroll container.
            The negative margin pulls the scrollbar outside the visible area */}
        <div className="overflow-y-auto pr-4" style={{ marginRight: "-16px", height: "100%" }}>
              ...
              return (
                <li key={item.id} className="relative">
                  {hasSubMenu ? (
                    <>
                      <div
                        className={`flex items-center gap-4 p-3 ${isToggled && "justify-center"} rounded-lg cursor-pointer transition-all duration-200 hover:bg-[var(--gradient-start)] hover:scale-105`}
                      ...
                      >
                        {subMenus[item.id].map((subItem, idx) => (
                          <li key={idx}>
                            <Link
                              to={`/students/${subItem.toLowerCase().replace(" ", "_")}`}
                    ...
                    <Link
                      to={`/students/${item.link === "dashboard" ? "" : item.link}`}
                      className={`flex items-center gap-4 p-3 ${isToggled && "justify-center"} rounded-lg cursor-pointer transition-all duration-200 hover:bg-[var(--gradient-start)] hover:scale-105`}
                    >
                      <span
                        className="bg-white/20 p-2 rounded-lg text-center transition-all hover:bg-white/30"
                      ...
                      {!isToggled && <span className="text-md font-medium">{item.name}</span>}
                    </Link>
                  )}
                </li>
              );

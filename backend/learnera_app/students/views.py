...
from .serializers import (
    ...
    StudentLeaveRequestDetailSerializer,
    ...
    def get_queryset(self):
        ...
        return (
            Assignment.objects.filter(
                class_section=student.class_assigned, is_active=True, status="published"
            )
            .select_related("subject", "class_section", "class_section__school_class")
    ...
    def post(self, request, assignment_id, *args, **kwargs):
        try:
            ...
            existing_submission = AssignmentSubmission.objects.filter(
                assignment=assignment, student=request.user.student, is_submitted=True
            ).first()
        ...
        if hasattr(self.request.user, "student"):
            ...
            try:
                return Exam.objects.get(
                    class_section=student.class_assigned, id=exam_id
                )
...
class StartExamView(APIView):
    ...
    def post(self, request, exam_id):
        ...
        if not exam.is_active():
            return Response(
                {"error": "Exam is not active"}, status=status.HTTP_400_BAD_REQUEST
            )

        student_exam, created = StudentExam.objects.get_or_create(
            student=student, exam=exam, defaults={"status": "NOT_STARTED"}
        )
...
class SubmitExamView(APIView):
    ...
    def post(self, request, exam_id):
        ...
        try:
            student_exam = StudentExam.objects.get(exam=exam, student=student)
        except StudentExam.DoesNotExist:
            return Response(
                {"error": "You haven't started this exam yet."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        student_exam.student_answers.all().delete()

        # Process new answers
        answer_data = request.data.get("answers", [])
        for answer in answer_data:
            question = get_object_or_404(Question, id=answer["question"])
            answer_data = {"question": question, "student_exam": student_exam}

            if question.question_type == "MCQ":
                selected_choice_id = answer.get("selected_choice")
                if selected_choice_id:
                    answer_data["selected_choice"] = get_object_or_404(
                        MCQChoice, id=selected_choice_id, question=question
                    )
            else:
                answer_data["answer_text"] = answer.get("answer_text", "")
    ...
    def get_queryset(self):
        if hasattr(self.request.user, "student"):
            return StudentExam.objects.filter(
                student=self.request.user.student, status="EVALUATED"
            ).select_related("exam")
...
class StudentDashboard(APIView):
    ...
    def get_attendance_data(self, student):
        ...
        attendance = (
            Attendance.objects.filter(student=student, date__year=current_year)
    ...
    def get_assignments_data(self, student):
        ...
        submissions = (
            AssignmentSubmission.objects.filter(student=student, is_submitted=True)
    ...
    def get_upcoming_deadlines(self, student):
        ...
        assignments = (
            Assignment.objects.filter(
                class_section=student.class_assigned, last_date__gte=now
            )
            .values("title", "last_date")
            .annotate(type=Count("id", filter=Q(id__gt=0)))[:3]
        )

        exams = (
            Exam.objects.filter(
                class_section=student.class_assigned, start_time__gte=now
            )
            .values("title", "start_time")
            .annotate(type=Count("id", filter=Q(id__gt=0)))[:2]
        )
    ...
    def get(self, request):
        ...
        response_data = {
            "student": {
                ...
                "profile_image": (
                    student.user.profile_image.url
            ...
            "recent_grades": {"average": recent_grades},
        }
